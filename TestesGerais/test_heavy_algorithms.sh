...
# Script para testar HORST, MSS e SPHINCS com número reduzido de execuções
...
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}  TESTE DE ALGORITMOS — HORST, MSS e SPHINCS    ${NC}"
echo -e "${BLUE}========================================${NC}"
...
# Para reativar no futuro, basta rodar com USAR_VALGRIND=1 ./test_heavy_algorithms.sh
...
(cd ../SPHINCS && make clean > /dev/null 2>&1 && make > /dev/null 2>&1)
if [ $? -ne 0 ]; then
    echo -e "${RED}Erro ao compilar SPHINCS!${NC}"
    exit 1
fi
echo -e "${GREEN}SPHINCS compilado com sucesso!${NC}"
...
# ── Função: testar SPHINCS ────────────────────────────────────────────────────
testar_sphincs() {
    local teste_num=$1
    echo -e "${BLUE}Testando SPHINCS-256 - Teste $teste_num/${TESTES}${NC}"
...
" "$teste_num" > ../SPHINCS/mensagem.txt
    ...
    local vg_metrics="0,0"
    if [ "$USAR_VALGRIND" -eq 1 ] && [ "$teste_num" -eq 1 ]; then
        echo -e "${YELLOW}  → Coletando métricas Valgrind (SPHINCS)...${NC}"
    ...
    echo -e "${GREEN}  ✓ SPHINCS $teste_num: Keygen=${tempo_secret_keys}s | Sign=${tempo_assinatura}s | Verif=${tempo_verificacao}s | Hashes=${hashes_assinatura}${NC}"
}

# ── Loop principal ─────────────────────────────────────────────────────────────
echo -e "${YELLOW}Iniciando $TESTES execuções para HORST, MSS e SPHINCS...${NC}"
echo -e "${YELLOW}Tempo estimado: HORST ~$((TESTES * 2))s | MSS ~$((TESTES * 12))s | SPHINCS ~$((TESTES * 2))s${NC}"
echo
